...
rats_raw %>%
  ...
  group_by(location_type, sighting_month_num, sighting_year, season) %>%
  mutate(rat_per_month = n()) %>%
  slice(1) %>%
  select(location_type, sighting_month_num, sighting_year, rat_per_month, season) %>% 
...
```{r, results = FALSE}
model <- lm(rat_per_month ~ sighting_year + sighting_month_num + location_type + season, data = rat_location)
summary(model)
```
...
```{r}
model_step = lm(rat_per_month ~ location_type + sighting_month_num + sighting_year + borough, data = rat_stepwise)
