...
test_bor <- broom::tidy(oneway.test(rat_per_month ~ sighting_month, data = rat_monthly))
```

We see that Brooklyn has the highest amount of rats, followed by Manhattan, the Bronx, Queens, and Staten Island. Running a one-way ANOVA test on the number of rat sightings (per month) by borough, we see that the p-value is significant (`r test_bor %>% pull(p.value)`), meaning that at least one of the borough means is not equal to the others.
